...
`npm test` 是 `&&` 串联链，任一环节失败即整体失败。全绿 = 131 断言：
...
| 6 | `node ../test_security_invariants.mjs` | 20 | **安全不变量**：错题卡脱敏缺席、register 独立桶、login-user-global 兜底、模拟考及格路径、多选 remap + 复习重映射、错题卡 5 类答案键剥离、rate_limit 桶、simulation_danger 负例 |
