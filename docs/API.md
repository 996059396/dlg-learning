...
#### POST /api/exam/start
- **双模式**（`POST /exam/start` 请求体或 query 传 `mode`，默认 `real`）：
  - `real` 全真：**100 题（60 判断 + 40 单选，无多选）/ 120 分钟 / 80 分及格** —— 对齐真实低压电工理论机考（应急〔2025〕59 号、应急管理部令第 19 号；官方附件未逐字公布题量配比，实际机考**随全国统一题库随机抽题**，60/40 为近似口径）。
  - `training` 训练：**100 题（60 判断 + 30 单选 + 10 多选）/ 45 分钟 / 80 分及格** —— 原格式，多选作训练用途。
- 题源：考证课程 `electrician_exam`（s1–s13）全部判断/单选节点随机抽样；训练模式的多选从独立池 `backend/data/exam/multi_select.json`（39 题，选项 A–D 带 `is_correct`）随机取 10。
...
- 响应：`{ sessionId, mode, total:100, minutes, passScore:80, expiresAt, questions:[脱敏题] }`。
