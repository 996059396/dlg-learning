...
## v1.0.3 (2026-08-17) — 稳定版

- **教材事实订正（crosscheck7 S05/S06 P0）**：`u4_relays` 8 脚时间继电器「1/4=复位端子」与厂商说明书冲突（8 脚 JS14S 无复位端子，短接 1/4 有内部短路风险）——配对题改 3 对、说明改为「引脚布局因型号而异、以说明书为准」；`u5_multimeter_advanced` CAT 抗浪涌排序订正为 IEC 61010-1 口径（CAT IV 600V 与 CAT III 1000V 同为 8000V 试验、CAT II 1000V=6000V），删去错误的「IV 600V > III 1000V」不等式。
- **SM-2 复习体系完整闭环**：四档评分（easy/hard/good 激活 easiness）、每日新增预算（`DAILY_NEW_CAP=20`）、每日复习负载均衡（`DAILY_REVIEW_CAP=30` 只服务最紧迫、其余惰性顺延）、leech 顽固错题标签与优先、已掌握卡 90 天复检、复习侧学习统计（`/game/mistakes/stats`：young/mature 留存率 + 7 天到期预报，Profile 面板）。
- **使用手册**：新增 `docs/使用手册.md` 面向使用者（启动/学习/练习/错题/模拟考/离线/稳定版状态）。
- **门禁基线**：182 断言全绿 + 判分 3748 节点 100%（3747 负例全拒）+ 5029 节点 0 ERROR + 706 课时零孤儿；前端 build 通过。

## v1.0.2 (未发布) — docs 四大件 + e2e 鉴权修复 + 数据卫生

- **docs 四大件收官（#98）**：`docs/API.md`（接口全契约）/ `docs/数据库schema.md`（13 表 + 迁移策略）/ `docs/课程设计.md`（3 课程 704 课时 / 5020 节点、12 种题型分布、知识图谱 586 条汇总、引用链 137 条）/ `docs/运维手册.md`（单实例约束、env 全表、备份回滚、升级流程）。40 项标准号联网核验（openstd.samr.gov.cn / IEC webstore），勘误 17 处，含两处硬错误：**GB 13398-2008 实为绝缘管棒标准（验电器应为 DL/T 740-2014）**、**GB/T 13870.1 现行为 2022 版**。
...
- **顺延 backlog 关板（2026-08-16）**：crosscheck4 P03 指出「Phase1-4 全部完成」声明与 14 项清单脱节、约 11 项仍 OPEN。现已全部关闭：幂等键（client_request_id，三路径重放无重复铸币）✓、Node24 文档 ✓、前端 vitest（19 用例 CI 三 OS）✓、guard 单实例锁 ✓、每日备份 + 升级前快照 ✓、多选池扩至 39 ✓、IME isComposing 守卫 ✓、对比度 token 化（#58CC02→#2A7E00 5.14:1）✓、demo 静默降级改显式 ✓、SimulationDial 候选集（查证仅 5-6 选项，伪问题）✓、乐观回滚（悲观式 await 后更新，无需回滚）✓。剩余未决为外部/产品项：named tunnel 固定域名、移动端真机、ICP/云主机、微信小程序。
